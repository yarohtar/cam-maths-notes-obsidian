...
Now also take $\beta(i)\in I_{\alpha(i)}$ such that $\tilde{f}_{0}(y_{0})\in V_{\beta(i)}$, find $T_{i}$ to be an open nbd of $y_{0}$ in $V_{\beta(i)}$ and set $W_{y_{0}}=\bigcup_{i}W_{y_{0}}^{(i)}\cap \tilde{f}_{0}^{-1}(T_{i})$
